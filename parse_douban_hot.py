...
def load_hot_data():
    """加载现有的热门数据"""
    try:
        if os.path.exists(HOT_MOVIES_FILE):
            with open(HOT_MOVIES_FILE, 'r', encoding='utf-8') as f:
                return json.load(f)
    except Exception as e:
        print(f"加载现有数据失败: {e}")
...
def get_douban_tv(cookie):
    ...
    if response.status_code == 200:
        ...
        raise requests.RequestException(f"获取电视剧数据失败: HTTP {response.status_code}")
...
def save_hot_data(data):
    ...
    with open(HOT_MOVIES_FILE, 'w', encoding='utf-8') as f:
...
def get_douban_movies(cookie):
    ...
    if response.status_code == 200:
        ...
        raise requests.RequestException(f"获取电影数据失败: HTTP {response.status_code}")
